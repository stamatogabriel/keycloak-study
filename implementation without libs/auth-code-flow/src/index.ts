...
app.get("/login", (req, res) => {
  ...
  const url = `http://localhost:8080/realms/full-cycle/protocol/openid-connect/auth?${loginParams.toString()}`;
  console.log(url);
  res.redirect(url);
});
...
app.get("/logout", (req, res) => {
  ...
  const url = `http://localhost:8080/realms/full-cycle/protocol/openid-connect/logout?${logoutParams.toString()}`;
  res.redirect(url);
});
...
app.get("/callback", async (req, res) => {
  ...
  const url = `http://keycloak:8080/realms/full-cycle/protocol/openid-connect/token`;
